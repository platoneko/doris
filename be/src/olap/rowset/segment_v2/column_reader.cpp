...
ColumnReader::ColumnReader(const ColumnReaderOptions& opts, const ColumnMetaPB& meta,
                           uint64_t num_rows, io::FileSystem* fs, const std::string& path)
        : _meta(meta), _opts(opts), _num_rows(num_rows), _fs(fs), _path(path) {}
...
Status ColumnReader::init() {
    ...
    for (int i = 0; i < _meta.indexes_size(); i++) {
        ...
        default:
            return Status::Corruption(strings::Substitute(
                    "Bad file $0: invalid column index type $1", _path, index_meta.type()));
        }
    }
    ...
    if (_ordinal_index_meta == nullptr && !is_empty()) {
        return Status::Corruption(strings::Substitute(
                "Bad file $0: missing ordinal index for column $1", _path, _meta.column_id()));
    }
    return Status::OK();
}
